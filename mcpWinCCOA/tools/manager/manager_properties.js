...
export function registerTools(server, context) {
    server.tool("get-manager-properties", `Get the configuration properties of a specific manager.
...
- Get properties of manager at position 5: managerIndex=5`, {
        ...
        try {
            ...
            return createSuccessResponse(response, `Properties for manager ${managerName} retrieved successfully`);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.error('Error getting manager properties:', errorMessage);
            return createErrorResponse(`Failed to get manager properties: ${errorMessage}`, { code: 'PMON_PROP_GET_ERROR' });
        }
    });
    server.tool("update-manager-properties", `Update the configuration properties of a specific manager.
...
- Update manager 5 to always restart: managerIndex=5, startMode="always", secKill=30, restartCount=3, resetMin=5, options="-f script.ctl"`, {
        ...
        try {
            ...
            const result = await pmonClient.updateManagerProperties(managerIndex, startMode, secKill, restartCount, resetMin, options || '');
            if (!result.success) {
                return createErrorResponse(`Failed to update manager properties at index ${managerIndex}: ${result.error}`, { code: 'PMON_PROP_PUT_ERROR', details: result.error });
            }
            const response = {
                managerIndex,
                managerName,
            ...
            return createSuccessResponse(response, `Properties for manager ${managerName} updated successfully`);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.error('Error updating manager properties:', errorMessage);
            return createErrorResponse(`Failed to update manager properties: ${errorMessage}`, { code: 'PMON_PROP_PUT_ERROR' });
        }
    });
    return 2; // Number of tools registered
}
//# sourceMappingURL=manager_properties.js.map