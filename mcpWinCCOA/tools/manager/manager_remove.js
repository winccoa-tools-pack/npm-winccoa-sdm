...
export function registerTools(server, context) {
    server.tool("remove-manager", `Remove a manager from the WinCC OA Pmon configuration.
...
- Remove manager at position 5: managerIndex=5`, {
        ...
        try {
            ...
            if (!result.success) {
                return createErrorResponse(`Failed to remove manager at index ${managerIndex}: ${result.error}`, { code: 'PMON_REMOVE_ERROR', details: result.error });
            }
            const response = {
                managerIndex,
                message: result.data
            ...
            return createSuccessResponse(response, `Manager ${managerName} removed successfully`);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.error('Error removing manager:', errorMessage);
            return createErrorResponse(`Failed to remove manager: ${errorMessage}`, { code: 'PMON_REMOVE_ERROR' });
        }
    });
    return 1; // Number of tools registered
}
//# sourceMappingURL=manager_remove.js.map