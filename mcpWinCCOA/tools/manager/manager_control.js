...
export function registerTools(server, context) {
    server.tool("start-manager", `Start a WinCC OA manager.
...
- Start manager at position 5: managerIndex=5`, {
        ...
        try {
            ...
            if (!result.success) {
                return createErrorResponse(`Failed to start manager at index ${managerIndex}: ${result.error}`, { code: 'PMON_START_ERROR', details: result.error });
            }
            const response = {
                managerIndex,
                message: result.data
            ...
            return createSuccessResponse(response, `Manager ${managerName} started successfully`);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.error('Error starting manager:', errorMessage);
            return createErrorResponse(`Failed to start manager: ${errorMessage}`, { code: 'PMON_START_ERROR' });
        }
    });
    server.tool("stop-manager", `Stop a WinCC OA manager gracefully.
...
- Stop manager at position 5: managerIndex=5`, {
        ...
        try {
            ...
            if (!result.success) {
                return createErrorResponse(`Failed to stop manager at index ${managerIndex}: ${result.error}`, { code: 'PMON_STOP_ERROR', details: result.error });
            }
            const response = {
                managerIndex,
                message: result.data
            ...
            return createSuccessResponse(response, `Manager ${managerName} stopped successfully`);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.error('Error stopping manager:', errorMessage);
            return createErrorResponse(`Failed to stop manager: ${errorMessage}`, { code: 'PMON_STOP_ERROR' });
        }
    });
    server.tool("kill-manager", `Force kill a WinCC OA manager immediately.
...
- Kill blocked manager at position 5: managerIndex=5`, {
        ...
        try {
            ...
            if (!result.success) {
                return createErrorResponse(`Failed to kill manager at index ${managerIndex}: ${result.error}`, { code: 'PMON_KILL_ERROR', details: result.error });
            }
            const response = {
                managerIndex,
                message: result.data
            ...
            return createSuccessResponse(response, `Manager ${managerName} killed successfully`);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.error('Error killing manager:', errorMessage);
            return createErrorResponse(`Failed to kill manager: ${errorMessage}`, { code: 'PMON_KILL_ERROR' });
        }
    });
    return 3; // Number of tools registered
}
//# sourceMappingURL=manager_control.js.map