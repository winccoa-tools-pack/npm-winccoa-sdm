...
export function registerTools(server, context) {
    server.tool("add-manager", `Add a new manager to the WinCC OA Pmon configuration.
...
- Add Archive: managerName="WCCOAvalarch", position=3, startMode="always", options="-num 1"`, {
        ...
        try {
            ...
            if (!cleanManagerName) {
                return createErrorResponse('Manager name cannot be empty', { code: 'INVALID_MANAGER_NAME' });
            }
            // Remove .exe extension if present
            const normalizedManagerName = cleanManagerName.replace(/\.exe$/i, '');
            const pmonClient = new PmonClient();
            const result = await pmonClient.addManager(position, normalizedManagerName, startMode, secKill, restartCount, resetMin, options);
            if (!result.success) {
                return createErrorResponse(`Failed to add manager: ${result.error}`, { code: 'PMON_ADD_ERROR', details: result.error });
            }
            const response = {
            ...
            return createSuccessResponse(response, `Manager '${normalizedManagerName}' added successfully at position ${position}`);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.error('Error adding manager:', errorMessage);
            return createErrorResponse(`Failed to add manager: ${errorMessage}`, { code: 'PMON_ADD_ERROR' });
        }
    });
    return 1; // Number of tools registered
}
//# sourceMappingURL=manager_add.js.map