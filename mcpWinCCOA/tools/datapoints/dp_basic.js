...
Supports all WinCC OA datapoint element types including state values, command values, parameters, alerts, and configuration elements.`, {
        ...
        try {
            ...
            if (!validation.valid) {
                return createErrorResponse(`Wildcard '*' not allowed in dpGet (response too large). Invalid datapoint element(s): ${validation.invalid.join(', ')}. Use get-datapoints tool with dpNamePattern for wildcard searches.`, {
            ...
            if (error.code === 71) {
                return createErrorResponse(`Datapoint does not exist. Please check the datapoint names. Error: ${errorMessage}`, {
...
//# sourceMappingURL=dp_basic.js.map