 ...
 */
function getMinMaxForType(dpeType) {
    switch (dpeType) {
 ...
 */
async function configureBinaryAlert(winccoa, dpe, direction, alarmClass = 'alert.') {
 ...
 */
async function configureAnalogAlert(winccoa, dpe, thresholds, dpeType, direction, alarmClasses) {
 ...
 */
async function hasAlertConfig(winccoa, dpe) {
    try {
 ...
 */
async function deactivateAlert(winccoa, dpe) {
    try {
...
export function registerTools(server, context) {
    ...
    server.tool("alarm-set", `Set or update alarm configuration for a datapoint element in WinCC OA.
    ...
    `, {
        ...
        try {
            ...
            if (hasConfig && !parsedConfig.force) {
                return createErrorResponse(`Alert configuration already exists for ${parsedConfig.dpe}. Use force: true to overwrite.`);
            }
            ...
            // Analog alarm (numeric)
                ...
                await configureAnalogAlert(winccoa, parsedConfig.dpe, parsedConfig.thresholds, dpeType, parsedConfig.direction, parsedConfig.alarmClasses);
            }
            else {
...
//# sourceMappingURL=alarm_set.js.map