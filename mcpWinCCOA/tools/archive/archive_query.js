 ...
 */
async function queryArchiveData(winccoa, dpe, startTime, endTime, count = 0) {
    ...
    const results = allData.map((dpeData, index) => {
        const values = dpeData.values || [];
        const timestamps = dpeData.times || [];
        return {
            dpe: dpeArray[index],
            values: values,
            timestamps: timestamps.map((t) => t instanceof Date ? t.toISOString() : t),
            count: values.length
        };
    });
    // Calculate total values across all DPEs
    const totalValues = results.reduce((sum, r) => sum + r.count, 0);
    return {
...
export function registerTools(server, context) {
    ...
    server.tool("archive-query", `Query historical archived data from WinCC OA datapoint elements.
    ...
    `, {
        ...
        try {
            ...
            const result = await queryArchiveData(winccoa, parsedConfig.dpe, parsedConfig.startTime, parsedConfig.endTime, parsedConfig.count || 0);
            console.log('========================================');
...
//# sourceMappingURL=archive_query.js.map