 ...
 */
async function hasArchiveConfig(winccoa, dpe) {
    try {
 ...
 */
async function configureArchive(winccoa, dpe, archiveClass) {
...
export function registerTools(server, context) {
    ...
    server.tool("archive-set", `Set or update archive configuration for a datapoint element in WinCC OA.
    ...
    `, {
        ...
        try {
            ...
            if (hasConfig && !parsedConfig.force) {
                return createErrorResponse(`Archive configuration already exists for ${parsedConfig.dpe}. Use force: true to overwrite.`);
            }
            // Configure archive
...
//# sourceMappingURL=archive_set.js.map