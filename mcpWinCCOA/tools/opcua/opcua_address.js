...
export function registerTools(server, context) {
    ...
    server.tool("opcua-add-address-config", `Configures OPC UA address settings for a datapoint element.
    ...
    Returns: Success status with configuration details.`, {
        ...
        try {
            ...
            const success = await opcua.addAddressConfig(params.dpName, params.connectionName, params.reference, datatype, direction, active, params.managerNumber, subscription);
            if (success) {
...
//# sourceMappingURL=opcua_address.js.map