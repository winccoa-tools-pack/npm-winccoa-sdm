...
export function registerTools(server, context) {
    ...
    server.tool("opcua-add-connection", `Creates and configures a new OPC UA client connection to an OPC UA server.
    ...
    server.tool("opcua-browse", `Browses the OPC UA address space with smart auto-depth selection.
    ...
    }, async ({ connectionName, parentNodeId, eventSource, depth, offset, limit, useCache, refreshCache }) => {
        try {
            // Validate depth parameter if specified
            if (depth !== undefined && (depth < 1 || depth > 5)) {
                return createErrorResponse(`Invalid depth parameter: ${depth}. ` +
            ...
            const browseResult = await opcua.browse(connectionName, parentNodeId, eventSourceNum, depth, // Pass undefined through for auto-depth
            useCache ?? true, refreshCache ?? false, undefined, // maxNodeCount (use default 800)
            offset ?? 0, limit);
    ...
    server.tool("opcua-delete-connection", `Completely deletes an existing OPC UA client connection from WinCC OA.
    ...
    }, async ({ connectionName, managerNumber }) => {
        try {
...
//# sourceMappingURL=opcua_connection.js.map