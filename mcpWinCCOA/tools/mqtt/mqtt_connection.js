 ...
 */
import { z } from "zod";
import { MqttConnection, MqttConnectionType, } from "../../helpers/drivers/MqttConnection.js";
...
export function registerTools(server, context) {
    ...
    // ============================================================================
    server.tool("mqtt-add-connection", `Creates and configures a new MQTT client connection to an MQTT broker.
    ...
    Returns: The auto-generated connection name on success.`, {
        ...
        try {
            ...
            if (connType === MqttConnectionType.TLS && !params.certificate) {
                return createErrorResponse("Certificate is required for TLS connections (connectionType=2)");
            }
            // Validate TLS-PSK requires pskIdentity and psk
            if (connType === MqttConnectionType.TLS_PSK) {
                if (!params.pskIdentity || !params.psk) {
                    return createErrorResponse("pskIdentity and psk are required for TLS-PSK connections (connectionType=4)");
                }
            }
            ...
            if (!result.success) {
                return createErrorResponse(`Failed to add MQTT connection: ${result.error}`);
            }
            const connTypeNames = {
                1: "Unsecure",
                2: "TLS",
            ...
            console.log(`Successfully created MQTT connection: ${result.connectionName}`);
            ...
            return createErrorResponse(`Failed to add MQTT connection: ${errorMessage}`);
        }
    });
    // ============================================================================
    // Tool 3: Delete MQTT Connection
    // ============================================================================
    server.tool("mqtt-delete-connection", `Deletes an existing MQTT connection.
    ...
    Returns: Success status.`, {
        ...
        try {
            ...
            if (!result.success) {
                return createErrorResponse(`Failed to delete MQTT connection: ${result.error}`);
            }
            return createSuccessResponse({
            ...
            return createErrorResponse(`Failed to delete MQTT connection: ${errorMessage}`);
        }
    });
    // ============================================================================
    // Tool 4: List MQTT Connections
    // ============================================================================
    server.tool("mqtt-list-connections", `Lists all configured MQTT connections and their current states.
    ...
    No parameters required.`, {}, async () => {
        try {
            console.log("Listing MQTT connections");
            const result = await mqtt.listConnections();
            if (!result.success) {
                return createErrorResponse(`Failed to list MQTT connections: ${result.error}`);
            }
            return createSuccessResponse({
            ...
            return createErrorResponse(`Failed to list MQTT connections: ${errorMessage}`);
        }
    });
    // ============================================================================
    // Tool 5: Get MQTT Connection State
    // ============================================================================
    server.tool("mqtt-get-connection-state", `Gets the current state of an MQTT connection.
    ...
    // ============================================================================
    server.tool("mqtt-add-address", `Configures an MQTT peripheral address for a datapoint element (DPE).
    ...
    Returns: Success status.`, {
        ...
        try {
            ...
            return createErrorResponse(`Failed to configure MQTT address: ${errorMessage}`);
        }
    });
    return 5;
}
//# sourceMappingURL=mqtt_connection.js.map