...
import { MqttConnectionType, MqttConnectionState, MqttProtocolVersion, MqttSslVersion, MqttAddressDirection, MqttTransformation, MQTT_DEFAULTS } from '../../types/index.js';
...
export class MqttConnection extends BaseConnection {
     ...
     */
    async getDefaultMqttDriverNumber() {
     ...
     */
    async ensureMqttDriverRunning(managerNumber) {
        const warnings = [];
        const pmonClient = new PmonClient();
        try {
            ...
            if (!driverFound) {
                ...
                for (const driverName of mqttDriverNames) {
                    ...
                    const addResult = await pmonClient.addManager(nextIndex, driverName, 'always', 30, 3, 5, `-num ${managerNumber}`);
    ...
    async addConnection(config) {
        try {
            ...
            const port = parseInt(connParts[1], 10);
            ...
            await this.winccoa.dpSetWait([`${connectionName}.Config.JsonProfiles`], [jsonProfiles]);
            ...
            if (enableConnection) {
                ...
                await this.winccoa.dpSetWait([`${connectionName}.Config.EstablishmentMode`], [1]);
                // Configure CheckConn address using base class method (sets _distrib + _address)
                const checkConnSuccess = await this.setAddressAndDistribConfig(`${connectionName}.Config.CheckConn`, {
                ...
                await this.winccoa.dpSetWait([`${connectionName}.Command.Enable`], [true]);
     ...
     */
    async getConnectionState(connectionName) {
        try {
            const dpName = connectionName.startsWith('_') ? connectionName : `_${connectionName}`;
            if (!this.checkDpExists(dpName)) {
                return { success: false, error: `Connection ${dpName} does not exist` };
            }
            const state = await this.winccoa.dpGet(`${dpName}.State.ConnState`);
            const stateTexts = {
                0: 'Inactive',
                1: 'Disconnected',
                2: 'Connecting',
    ...
    async listConnections() {
        try {
            ...
            const dpNames = await this.winccoa.dpNames('*', '_MqttConnection');
            ...
            for (const dpName of dpNames) {
                ...
                if (dpName.endsWith('_2'))
                    continue;
                try {
                    const stateResult = await this.getConnectionState(dpName);
                    let connectionString;
                    try {
                        const addressJson = await this.winccoa.dpGet(`${dpName}.Config.Address`);
                        if (addressJson) {
    ...
    async addAddressConfig(params) {
        ...
        if (!this.checkDpExists(connectionName)) {
            throw new Error(`MQTT connection ${connectionName} does not exist. ` +
        ...
        if (!driverNumber) {
            try {
                driverNumber = await this.winccoa.dpGet(`${connectionName}.Config.DrvNumber`);
            }
            catch {
        ...
        if (params.qos !== undefined)
            console.log(`  - QoS: ${params.qos}`);
        if (params.retain !== undefined)
            console.log(`  - Retain: ${params.retain}`);
        if (params.oldNewComparison !== undefined)
            console.log(`  - Old/New Comparison: ${params.oldNewComparison}`);
        console.log(`╚════════════════════════════════════════════════════════════════╝\n`);
        // Use setAddressAndDistribConfig from BaseConnection
        const success = await this.setAddressAndDistribConfig(params.dpeName, {
            _type: DpConfigType.DPCONFIG_PERIPH_ADDR_MAIN,
            _drv_ident: 'MQTT',
            _connection: connectionName,
            _reference: topic,
