...
export class PmonClient {
     ...
     */
    async addManager(index, manager, startMode = 'always', secKill = 30, restartCount = 3, resetMin = 5, options = '') {
        try {
            // Validate inputs
     ...
     * @returns Promise with operation result
     */
    async stopManager(index, ownManagerNumber) {
        try {
            if (index < 1) {
                throw new Error('Manager index must be at least 1');
            }
     ...
     * @returns Promise with operation result
     */
    async killManager(index, ownManagerNumber) {
        try {
            if (index < 1) {
                throw new Error('Manager index must be at least 1');
            }
     ...
     */
    async getManagerProperties(index) {
        try {
            if (index < 1) {
                throw new Error('Manager index must be at least 1');
            }
     ...
     * @returns Promise with operation result
     */
    async updateManagerProperties(index, startMode, secKill, restartCount, resetMin, options = '') {
        try {
            if (index < 1) {
                throw new Error('Manager index must be at least 1');
            }
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            if (parts.length >= 5) {
                managers.push({
                    ...
                    state: parseInt(parts[0] || '0', 10),
                    pid: parseInt(parts[1] || '0', 10),
                    startMode: parseInt(parts[2] || '0', 10),
                    startTime: parts[3] || '',
     ...
     */
    parseManagerProperties(response) {
...
//# sourceMappingURL=PmonClient.js.map