 ...
 */
import { isGridPosition, getWidgetPresets, getDefaultDimensions, DASHBOARD_GRID } from '../../types/dashboards/layout.js';
/**
 * Layout Helper Class
 */
export class LayoutHelper {
     ...
     */
    resolveLayout(layout, widgetType, existingWidgets) {
     ...
     */
    getPresetDimensions(preset, widgetType) {
        ...
        if (!dimensions) {
            console.warn(`Preset '${preset}' not available for widget type '${widgetType}', using default`);
            return getDefaultDimensions(widgetType);
        }
        return dimensions;
    }
     ...
     */
    findAutoPosition(existingWidgets, widgetSize) {
     ...
     */
    isPositionAvailable(position, size, existingWidgets) {
        const newWidget = {
            x: position.x,
            y: position.y,
            cols: size.cols,
     ...
     */
    widgetsOverlap(w1, w2) {
        return !(w1.x + w1.cols <= w2.x || // w1 is left of w2
     ...
     */
    suggestNextPosition(existingWidgets, widgetType, preset = 'auto') {
