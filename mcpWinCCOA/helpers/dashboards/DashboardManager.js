...
import { getDashboardDatapointName as getDashboardDpName, extractDashboardNumber as extractDashboardNum, createDefaultDashboardSettings as createDefaultSettings } from '../../types/dashboards/dashboard.js';
...
export class DashboardManager {
    ...
    async createDashboard(config) {
        ...
        if (config.createdBy.toLowerCase() === 'root') {
            throw new Error('Dashboard cannot be created with "root" as creator. Dashboards created by root cannot be modified later. Please use a proper user account (e.g., "admin" or another valid username).');
        }
        // Lookup user and get userId
        const userNames = await this.winccoa.dpGet('_Users.UserName');
        const userIndex = userNames.findIndex((name) => name === config.createdBy);
        if (userIndex === -1) {
            throw new Error(`User not found: ${config.createdBy}. Dashboard cannot be created without valid creator.`);
        }
     ...
     */
    async editDashboard(dashboardId, updates) {
     ...
     */
    async addWidget(dashboardId, config) {
        // Verify dashboard exists
        if (!this.winccoa.dpExists(dashboardId)) {
            throw new Error(`Dashboard ${dashboardId} does not exist`);
        }
        ...
        // Resolve layout
        const dimensions = this.layoutHelper.resolveLayout(config.layout, config.type, existingWidgets);
        // Validate no overlaps (safety check for explicit layouts)
        const wouldOverlap = existingWidgets.some((existing) => {
            return this.layoutHelper.widgetsOverlap({ x: dimensions.x, y: dimensions.y, cols: dimensions.cols, rows: dimensions.rows }, existing);
        });
        if (wouldOverlap) {
            console.warn(`Warning: Widget at position (${dimensions.x}, ${dimensions.y}) may overlap with existing widgets. Consider using layout: "auto" instead.`);
        ...
        if (config.type === 'trend' && !config.yAxisUnit) {
            const firstDp = config.dataPoint ||
                (config.dataPoints?.[0] &&
                    (typeof config.dataPoints[0] === 'string'
                        ? config.dataPoints[0]
                        : config.dataPoints[0].dataPoint));
            if (firstDp) {
                const unit = await this.getDatapointUnit(firstDp);
                if (unit) {
                    config.yAxisUnit = unit;
     ...
     */
    async editWidget(dashboardId, identifier, updates) {
        // Get existing widgets
        ...
        if (updates.appearance) {
            ...
            const createMultilingualText = (text) => ({
                context: 'group',
                config: {
                    'en_US.utf8': text
                }
        ...
        const settings = widget.settings;
        const widgetUpdates = updates; // Cast to any for dynamic property access
        ...
        if (updates.layout) {
            const dimensions = this.layoutHelper.resolveLayout(updates.layout, widget.component.tagname.includes('gauge')
                ? 'gauge'
                    ...
                    : widget.component.tagname.includes('trend')
                        ...
                        : widget.component.tagname.includes('pie')
                            ...
                            : widget.component.tagname.includes('progress-bar')
                                ? 'progressbar'
                                : 'barchart', widgets.filter((_, i) => i !== widgetIndex));
     ...
     * @param identifier - Widget identifier (by ID or index)
     */
    async deleteWidget(dashboardId, identifier) {
        // Get existing widgets
        const widgets = await this.getWidgets(dashboardId);
        // Find widget
        const widgetIndex = this.findWidgetIndex(widgets, identifier);
     ...
     */
    findWidgetIndex(widgets, identifier) {
     ...
     * @param config - Widget configuration
     */
    async validateDatapoints(config) {
        const datapoints = [];
