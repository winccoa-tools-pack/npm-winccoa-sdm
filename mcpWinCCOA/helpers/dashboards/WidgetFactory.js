 ...
 */
import { createArrayContext, createSimpleGroupContext } from '../../types/dashboards/schema.js';
/**
...
export class WidgetFactory {
     ...
     */
    registerWidgetType(type, creator) {
     ...
     */
    createWidget(config, dimensions) {
     ...
     */
    createGeneralSettings(appearance) {
        const config = {
     ...
     * Create Gauge widget
     */
    createGauge(config, dimensions) {
        const { dataPoint, title, chartType = 'classic', showTooltip = true, color, format, unit, name, isRelative = false, 
        // Global appearance settings
        animation, font, renderer, theme, backgroundColor, 
     ...
     * Create Label widget
     */
    createLabel(config, dimensions) {
        const { dataPoint, title, icon = '', iconPosition = 'left', iconSizeFactor = 'medium', fontSizeFactor = 'medium', // Changed from 'small' to 'medium' for better readability
        unitFontSizeFactor = 'small', color, format, unit, name, 
        // Widget appearance (header, footer, icons, colors)
        appearance } = config;
        const { x, y, cols, rows, minCols, minRows } = dimensions;
        const settings = {
     ...
     */
    createTrend(config, dimensions) {
        const { dataPoint, dataPoints, title, timeRange = 'now/h', // Default to current hour (from last full hour) - grows with time for live monitoring
        legendType = 'scroll', legendOrientation = 'horizontal', legendVerticalPosition = 'top', legendHorizontalPosition = 'center', showLegend = true, showXAxisGrid = false, showYAxisGrid = true, showRangePicker = true, showTooltip = true, yAxisColor = '', yAxisName = '', yAxisUnit, range, zoom = 1, 
        // Global appearance settings
        animation, font, renderer, theme, backgroundColor, 
        // Widget appearance (header, footer, icons, colors)
        appearance } = config;
        ...
        const seriesArray = dpArray.map((dp) => {
            ...
            if (typeof dp !== 'string') {
                ...
                if (dp.showArea !== undefined)
                    seriesConfig.showArea = dp.showArea;
                if (dp.showConfidenceBand !== undefined)
                    seriesConfig.showConfidenceBand = dp.showConfidenceBand;
                if (dp.color)
                    seriesConfig.color = dp.color;
                // Data formatting
                if (dp.unit)
                    seriesConfig.unit = dp.unit;
                if (dp.format)
                    seriesConfig.format = dp.format;
                if (dp.name)
                    seriesConfig.name = dp.name;
                ...
                if (dp.showCustomYAxis) {
                    ...
                    if (dp.min !== undefined)
                        seriesConfig.min = dp.min;
                    if (dp.max !== undefined)
                        seriesConfig.max = dp.max;
                }
            }
        ...
        const settings = {
            ...
            config: createSimpleGroupContext({
                ...
                series: createArrayContext(seriesArray),
                showLegend,
                zoom,
                legendOrientation,
                showYAxisGrid,
     ...
     */
    createPie(config, dimensions) {
        const { dataPoints, dataPointsDescriptions, title, chartType = 'pie', labelsShow = false, labelsPosition = 'outside', labelsDetails = 'value', labelLineLength = 10, legendPosition = 'topright', showTooltip = true, colors, darkModeColors, 
        // Widget appearance (header, footer, icons, colors)
        appearance } = config;
        const { x, y, cols, rows, minCols, minRows } = dimensions;
        ...
        const seriesArray = dataPoints.map((dp, index) => createSimpleGroupContext({
            datapoint: {
                context: 'data-point',
                config: {
                    dpName: dp,
     ...
     */
    createProgressBar(config, dimensions) {
        const { dataPoint, title, color, size = '2.25em', unit, format, min, max, showRange = true, isAbsolute = false, alertRanges, 
        // Global appearance settings
        animation, font, renderer, theme, backgroundColor, 
        // Widget appearance (header, footer, icons, colors)
        appearance } = config;
     ...
     */
    createBarChart(config, dimensions) {
        const { dataPoints, title, yAxisName = '', yAxisUnit, yAxisColor, range, isStacked = false, isHorizontal = false, showTooltip = true, showLegend = true, legendPosition = 'topright', 
        // Global appearance settings
        animation, font, renderer, theme, backgroundColor, 
        // Widget appearance (header, footer, icons, colors)
        appearance } = config;
        ...
        const seriesArray = dataPoints.map((dp) => createSimpleGroupContext({
            datapoint: {
                context: 'data-point',
                config: {
                    dpName: dp,
